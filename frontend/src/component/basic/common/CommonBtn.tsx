...
const ConfirmBtn = styled(Button)<{ isBlack: boolean }>`
    background-color: ${(p) => (p.isBlack ? BLACK_COLOR : 'white')};
    color: ${(p) => (p.isBlack ? 'white' : BLACK_COLOR)};
    border-radius: 0.5rem;
    border: ${(p) => (p.isBlack ? `0.0rem solid ${BLACK_COLOR}` : `0.15rem solid ${BLACK_COLOR}`)};
    &:hover {
...
const CommonBtn = ({ isBlack, onClick, ...props }: PropsWithChildren<CommonBtnProps & ButtonProps>): JSX.Element => {
    return (
        <ConfirmBtn isBlack={isBlack} onClick={onClick} {...props}>
            {props.children}
        </ConfirmBtn>
    );
};
